...
void	go_draw(t_var v)
{
	...
	while (i < THREADS)
	{
		...
		if (ft_strcmp(v.fractol, "mandelbrot") == 0)
			pthread_create(&id[i], NULL, mandelbrot, (void *)&t[i]);
		else if (ft_strcmp(v.fractol, "julia") == 0)
			pthread_create(&id[i], NULL, julia, (void *)&t[i]);
		else if (ft_strcmp(v.fractol, "ship") == 0)
			pthread_create(&id[i], NULL, ship, (void *)&t[i]);
		else if (ft_strcmp(v.fractol, "quatro") == 0)
			pthread_create(&id[i], NULL, quatro, (void *)&t[i]);
		else
			pthread_create(&id[i], NULL, penta, (void *)&t[i]);
		v.t_start = t[i].t_end;
		i++;
	}
	i = 0;
